...
<div class="p-1 pb-0 ml-0 pl-0">
    <h5 class="mt-2 text-primary"> Chatbot questions (Upload Batch) </h5>
</div>
</div>
...
export default {
    ...
    onUpload: function(){
        ...
        axios.post(process.env.VUE_APP_API+'/upload/questions/preview', form, {
    ...
    upload: function(){
       this.$Progress.start()
        ...
        form.append('userid', this.usersession['id'])
        form.append('csrf_test_name', this.token)
        axios.post(process.env.VUE_APP_API+'/upload/questions/upload', form, {
        })
        .then(response => {
        if(response.data.status==response.data.statusmsg){
        this.classname=response.data.classname
